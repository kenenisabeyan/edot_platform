    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[460px] flex flex-col shadow-2xl transition-all duration-300 animate-in slide-in-from-right">
          ...
          {/* Messages Area */}
          <div className={`flex-1 overflow-y-auto p-5 space-y-6 ${isDarkMode ? 'bg-[#05070A]' : 'bg-slate-50'}`}>
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                {msg.sender === 'user' ? (
                  <div className="max-w-[85%] px-4 py-3 rounded-2xl bg-cyan-600 text-white font-medium text-sm shadow-md">
                    {msg.text}
                  </div>
                ) : (
                  <div className={`max-w-[95%] p-5 rounded-3xl border space-y-4 shadow-sm ${isDarkMode ? 'bg-[#0B1120] text-slate-100 border-white/10' : 'bg-white text-slate-800 border-slate-200'}`}>
                        ...
                        <div className="flex flex-col gap-1.5">
                          {msg.suggestedNextActions.map((act, idx) => (
                            <button
                              ...
                              className={`text-left text-xs font-semibold px-3 py-2 rounded-xl border transition-all flex items-center justify-between group ${isDarkMode ? 'bg-[#05070A] border-white/10 hover:border-cyan-500/40 text-slate-300 hover:text-white' : 'bg-slate-50 border-slate-200 hover:border-cyan-500 text-slate-700'}`}
                            >
                              <span>{act}</span>
          ...
          {/* Drawer Input Form */}
          <form onSubmit={handleSendMessage} className={`p-4 border-t ${isDarkMode ? 'bg-[#0B1120] border-white/10' : 'bg-white border-slate-200'}`}>
            <div className="relative flex items-center">
              <input
                ...
                className={`w-full px-4 py-3.5 rounded-2xl text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-cyan-500/50 pr-14 ${isDarkMode ? 'bg-[#05070A] text-white border-white/10' : 'bg-slate-100 text-slate-800 border-slate-200'}`}
              />
              <button
                type="submit"
                disabled={loading || !inputMessage.trim()}
                className="absolute right-2 p-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-600 disabled:opacity-50 text-white transition-all shadow-md"
              >
