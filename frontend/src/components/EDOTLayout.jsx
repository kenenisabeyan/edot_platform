...
export default function EDOTLayout() {
  ...
  const isIntelActive = location.pathname.startsWith('/dashboard/intelligence');

  const [financeOpen, setFinanceOpen] = useState(false);
  const [intelOpen, setIntelOpen] = useState(() => {
    const saved = localStorage.getItem('edot_intel_expanded');
  ...
  const toggleIntelOpen = () => {
    setIntelOpen((prev) => {
      const next = !prev;
      localStorage.setItem('edot_intel_expanded', JSON.stringify(next));
      return next;
    });
  };
        ...
        <div className="overflow-y-auto overflow-x-hidden p-3 space-y-6 flex-1 scrollbar-hide mt-3">
           ...
           <div>
             ...
             <nav className="space-y-2">
               ...
               {/* 🧠 Intelligence Hub Parent Item with 13 Collapsible Children */}
               <div>
                 <div
                   onClick={() => {
                     toggleIntelOpen();
                   ...
                   className={`group relative flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-300 text-[13px] cursor-pointer ${
                     isIntelActive
                       ? isDarkMode 
                         ? 'bg-[#22C55E]/10 text-[#22C55E] font-bold border-l-4 border-[#22C55E]' 
                         : 'bg-[#EAF6ED] text-[#22C55E] font-bold border-l-4 border-[#22C55E]'
                       : isDarkMode
                         ? 'text-slate-300 font-semibold hover:bg-slate-800 hover:text-slate-100'
                         : 'text-slate-700 font-semibold hover:bg-slate-50 hover:text-slate-900'
                   }`}
                 >
                   <div className="flex items-center gap-3">
                     <Brain className={`w-5 h-5 shrink-0 transition-transform group-hover:scale-110 duration-300 ${isIntelActive ? 'text-[#22C55E]' : 'text-purple-400'}`} />
                     {!sidebarCollapsed && <span className="animate-in fade-in slide-in-from-left-2 duration-300 font-bold">Intelligence Hub</span>}
                   </div>
                   {!sidebarCollapsed && (
                     <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${intelOpen ? 'rotate-180 text-[#22C55E]' : 'text-slate-400'}`} />
                   )}
                 </div>

                 {/* 13 Collapsible Child Items */}
                 {intelOpen && !sidebarCollapsed && (
                   <div className="pl-6 pr-2 py-1.5 space-y-1 transition-all duration-300 animate-in slide-in-from-top-2">
                     {[
                       { name: 'Your Next Best Step', path: '/dashboard/intelligence/next-step', icon: Sparkles },
                       { name: 'My Progress', path: '/dashboard/intelligence/progress', icon: TrendingUp },
                       { name: 'My Mastery', path: '/dashboard/intelligence/mastery', icon: Brain },
                       { name: 'My Skills', path: '/dashboard/intelligence/skills', icon: Target },
                       { name: 'AI Mentor', path: '/dashboard/intelligence/mentor', icon: Bot },
                       { name: 'Recommendations', path: '/dashboard/intelligence/recommendations', icon: Lightbulb },
                       { name: 'Learning Path', path: '/dashboard/intelligence/learning-path', icon: Compass },
                       { name: 'Learning Pulse', path: '/dashboard/intelligence/pulse', icon: Flame },
                       { name: 'Mentors & Collaboration', path: '/dashboard/intelligence/mentors', icon: Users },
                       { name: 'Projects & Experience', path: '/dashboard/intelligence/projects', icon: Rocket },
                       { name: 'Career Readiness', path: '/dashboard/intelligence/career', icon: Briefcase },
                       { name: 'Opportunities', path: '/dashboard/intelligence/opportunities', icon: Globe },
                       { name: 'Insights', path: '/dashboard/intelligence/insights', icon: BarChart3 }
                     ].map((child) => {
                         ...
                         <NavLink
                           ...
                           className={({ isActive }) =>
                             `flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs transition-all duration-200 ${
                               isActive
                                 ? isDarkMode
                                   ? 'bg-[#22C55E]/15 text-[#22C55E] font-extrabold border-l-2 border-[#22C55E]'
                                   : 'bg-[#EAF6ED] text-[#16a34a] font-extrabold border-l-2 border-[#22C55E]'
                                 : isDarkMode
                                   ? 'text-slate-400 font-semibold hover:text-slate-200 hover:bg-slate-800/60'
                                   : 'text-slate-600 font-semibold hover:text-slate-900 hover:bg-slate-100'
                             }`
                           }
                         >
